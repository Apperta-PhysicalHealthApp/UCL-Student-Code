import { assign, changesHandler, clone, filterChange, guardedConsole, hasLocalStorage, pick, toPromise, uuid } from 'pouchdb-utils';
import { isDeleted, isLocalId, parseDoc, preprocessAttachments, processDocs } from 'pouchdb-adapter-utils';
import { collectConflicts, compactTree, latest, traverseRevTree } from 'pouchdb-merge';
import { safeJsonParse, safeJsonStringify } from 'pouchdb-json';
import { binaryStringToBlobOrBuffer, btoa } from 'pouchdb-binary-utils';
import { Map, Set } from 'pouchdb-collections';
import { MISSING_DOC, MISSING_STUB, REV_CONFLICT, WSQ_ERROR, createError } from 'pouchdb-errors';
...
var websqlChanges = new changesHandler();
...
function WebSqlPouch(opts, callback) {
  ...
  api._getAttachment = function (docId, attachId, attachment, opts, callback) {
    ...
    tx.executeSql(sql, [digest], function (tx, result) {
      ...
      if (opts.binary) {
        res = binaryStringToBlobOrBuffer(data, type);
      } else {
        res = btoa(data);
      }
      callback(null, res);
